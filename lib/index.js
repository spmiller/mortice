...
// TODO: id and generateId are just for testing purposes. Do not submit in PR
let id = 1;
const generateId = () =>  id++
const createMutex = (name, options) => {
  ...
  return {
    ...
    isIdle: () => {
      return !masterQueue.pending && !masterQueue.size
    }
  }
}
...
const defaultCreateMutex = (name, options) => {
  ...
  if (implementation.isWorker) {
    // don't keep the mutex for worker
...
const mutexesMutation = defaultCreateMutex('mutexesMutation')
const managedLockOfType = async function (lockType, name, options) {
  const mutexesMutationRelease = await mutexesMutation.writeLock()
  if (!mutexes[name]) {
    mutexes[name] = defaultCreateMutex(name, options)
  }

  const mutex = mutexes[name]

  const mutexRelease = await mutex[lockType]()
  await mutexesMutationRelease()
  return async () => {
    await mutexRelease()
